...
namespace SpaceGraphicsToolkit.Nebula
{
	...
	public class SgtNebula : MonoBehaviour, CwChild.IHasChildren
	{
		...
		[ContextMenu("Rebuild")]
		private float CustomNoise(Vector3 point)
		{
			// Scale the input point to avoid getting the same value for integer inputs
			point *= 0.1f;
			// Use Perlin noise and map it from [0, 1] to [-1, 1]
			return Mathf.PerlinNoise(point.x, point.y) * 2.0f - 1.0f;
		}
		...
		public void Rebuild()
		{
			...
			if (sourceMesh != null)
			{
				...
				{
					var normal = generatedPositions[i].normalized;

					// Replace Unity.Mathematics.noise.snoise with a custom noise function
					generatedPositions[i] = normal * (radius + displacement * CustomNoise(normal * frequency));
				}
